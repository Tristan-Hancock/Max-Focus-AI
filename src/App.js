...
if(error){
  ...
  async function callOpenAIAPI() {
    ...
    setShowConfirmationModal(false); // Ensure confirmation modal is reset for new task submissions
    setShowModal(false); // Ensure modal is reset for new task submissions
    setEventAdded(false); // Reset the event added state
  ...
  async function createCalendarEvent() {
    setIsLoading(true); // Indicate the start of an operation
    setEventAdded(false); // Reset the event added state at the start of a new operation
    setShowModal(false); 
    ...
    try {
      ...
      console.log(data);
      setEventAdded(true); // Indicate that an event has been added successfully
    } catch (error) {
      console.error('Error during event creation:', error);
      alert("Failed to create event. Please try again."); // Optionally, inform the user of the failure
    } finally {
      setIsLoading(false); // Reset the loading state at the end of the operation
      setShowModal(false); // Close the modal whether the operation was successful or not
      setEventName("");
    }
  }
  ...
  <meta http-equiv="Content-Security-Policy" content="
    ...
    img-src 'self' https://*.supabase.co https://www.example.com;
  " />
</Helmet>
...
{showModal && !eventAdded && !isLoading && (
 ...
 { /*<DateTimePicker  value={start} onChange={setStart}  />
  <DateTimePicker  value={start} onChange={setEnd}  />*/   } 
            ...
            <hr />
            <div className="enter"> 
            <button class="button " onClick={() => {if (eventName.trim() === '') {alert('Please enter an event name.');} else {createCalendarEvent();}}} >
  ...
  Add to Calendar </button>
    ...
    <a href="https://twitter.com/TristanInTech" class="socialContainer containerTwo">
